#!/usr/bin/env python3
"""Verify the signed nightly APK's package metadata and signer."""
...
def main() -> int:
    ...
    if args.apk.name != "bitchat-nightly-universal.apk":
    ...
    if not labels or any(label != "bitchat nightly" for label in labels):
        raise RuntimeError(f"unexpected application labels: {sorted(set(labels))}")

    signer = run([str(args.apksigner), "verify", "--verbose", "--print-certs", str(args.apk)])
    match = re.search(r"certificate SHA-256 digest:\s*([0-9a-fA-F:]{64,95})", signer)
    if match is None:
    ...
    if actual_cert != expected_cert:
        raise RuntimeError(f"signer mismatch: expected {expected_cert}, got {actual_cert}")
    print(
        f"verified {args.apk.name}: {APPLICATION_ID}, bitchat nightly, "
